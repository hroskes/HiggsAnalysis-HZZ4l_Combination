...
def parseOptions():
    ...
    parser.add_option('-m', '--model', type='string', dest='model', default="Nophase" ,help='model, phase or Nophase')
    parser.add_option('-f', '--interference', type="int", dest='interf', default=1 ,help='include interference')
...
def creationLoop(directory):
	...
	c = 0
        while (c < endVal[a] ): 
            ...
            myClass.makeCardsWorkspaces(mh,directory,theInputs4e,opt.templateDir,opt.interf)
            myClass.makeCardsWorkspaces(mh,directory,theInputs4mu,opt.templateDir,opt.interf)
            myClass.makeCardsWorkspaces(mh,directory,theInputs2e2mu,opt.templateDir,opt.interf)
