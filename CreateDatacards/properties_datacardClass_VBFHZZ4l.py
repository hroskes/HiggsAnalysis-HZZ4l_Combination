...
class properties_datacardClass_VBFHZZ4l(MultiEnum):
    enums = (Analysis, Category, Channel, Production)
    mH = 125
    ...
    def makeCardsWorkspaces(self, unused, theOutputDir, *moreunused):
        ...
        outdir = os.path.join(theOutputDir, "HCG", str(self.mH))

        filebase = "hzz4l_{}S_{}_{}".format(self.channel,self.category,self.production.year)
        txtfile = filebase+".txt"
        rootfile = filebase+".root"

        w = ROOT.RooWorkspace("w","w")

        w.importClassCode(HZZ4L_RooSpinZeroPdf.Class(),True)
        w.importClassCode(VBFHZZ4L_RooSpinZeroPdf.Class(),True)
        ...
        w.writeToFile(os.path.join(outdir, rootfile))
